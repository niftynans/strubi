...
class CoCo:
    def __init__(self,
        ...
        print("Checking graph cut path..")
        if dag is not None:
            print("oracle similarities")
            self._oracle_similarities(dag)
            print("oracle graph cut")
            self._oracle_graph_cuts()
            if self.known_number_confounders:
                print("confounder known")
                self._oracle_graph_cuts_n(n_components)

        print("estimated graph cut")
        self._estimated_graph_cuts()
        if self.known_number_confounders:
            print("confounder known after estimation")
    ...
    def _discover_dag(self, G, dag_discovery, soft=True):
        ...
        self.G_estimated = G_
        print(f"(COCO-dag) Discovered DAG edges: {list(G_.edges())}")
        return G_
    ...
    def _score_DAG(self, G):
        ...
        for node_i in self.nodes:
            print(f"Testing node {node_i} with parents {[n for n in G.predecessors(node_i)]}...")
    ...
    def _estimated_similarities(self):
        ...
        # === Similarity Metrics ===
        # MI                : {self.sim_mi}
        # Binary (01)       : {self.sim_01}
        # P-Value           : {self.sim_pval}
        # Centrality        : {self.sim_cent}
        # Causal Binary     : {self.sim_causal_01}
        # Causal P-Value    : {self.sim_causal_pval}
        # """
        # )
    ...
    def confounded_components_to_labels(self, confounded_components):
        ...
        index = np.array([i for i in self.nodes])
