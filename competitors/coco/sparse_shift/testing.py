...
def _compute_mechanism_pair(e1, e2, Xs, m, parents, test, test_kwargs):
    """Worker function to compute a single pair of environments"""
    if sum(parents) == 0:
        from hyppo.ksample import MMD
        stat, pvalue = MMD().test(
            Xs[e1][:, m].reshape(-1, 1),
        ...
        if test == 'kcd':
            assert len(Xs) == 2
            # CRITICAL: Force inner n_jobs=1 to prevent catastrophic nested parallelism
            _, pvalue = KCD(n_jobs=1).test(
                np.vstack((Xs[e1][:, parents], Xs[e2][:, parents])),
                np.concatenate((Xs[e1][:, m], Xs[e2][:, m])),
                np.asarray([0] * Xs[e1].shape[0] + [1] * Xs[e2].shape[0]),
                reps=test_kwargs.get('n_reps', 100),
            )
        elif test == 'invariant_residuals':
            assert len(Xs) == 2
            pvalue, *_ = invariant_residual_test(
...
def test_mechanism_fast(Xs, m, parents, test='kci', test_kwargs={}, n_jobs=-1):
    ...
    pairs = [(e1, e2) for e1 in range(E) for e2 in range(e1 + 1, E)]

    results = Parallel(n_jobs=n_jobs)(
        delayed(_compute_mechanism_pair)(e1, e2, Xs, m, parents, test, test_kwargs)
        for e1, e2 in pairs
    )

    for e1, e2, pvalue in results:
        pvalues[e1, e2] = pvalue
        pvalues[e2, e1] = pvalue
