...
def example_mini():
    ...
    # parameters to test
    n_nodes, n_confounders, n_contexts, n_samples = (5, 1, 15, 500)
    ...
    G_obs = dag.G
    adj_matrix = nx.to_numpy_array(G_obs, nodelist=sorted(G_obs.nodes()))
    pos = nx.spring_layout(dag.G, seed=seed, k=0.8)
    plt.figure(figsize=(10, 8))
    nx.draw_networkx_nodes(G_obs, pos, 
                    node_size=700, 
                    node_color='skyblue', 
                    edgecolors='navy')

    nx.draw_networkx_labels(G_obs, pos, font_size=12, font_family='sans-serif')
    nx.draw_networkx_edges(G_obs, pos, 
                        edgelist=G_obs.edges(),
                        edge_color='gray', 
                        arrows=True, 
                        arrowsize=20,
                        arrowstyle='-|>',
    ...
    plt.title("Directed Graph Visualization from Adjacency Matrix", fontsize=15)
    plt.axis('off') 
    plt.tight_layout()
    plt.show()
