...
def reproduce_fig3(path="coco/experiments/data_cytometry"):
    ...
    nms = Dc[0].columns
    Dc = [np.array(X[:707]) for X in Dc]
    # FOR SELECTION ON PKA
    pkc_col_name = 'cd3cd28.PKC'
    pkc_idx = list(nms).index(pkc_col_name)

    Dc_filtered = []

    for X in Dc:
        pkc_values = X[:, pkc_idx]
        threshold = np.percentile(pkc_values, 80)
        
        mask = pkc_values >= threshold
        
        Dc_filtered.append(X[mask])

    for i, X_filt in enumerate(Dc_filtered):
        print(f"Context {i+1} original: 707 rows | Filtered: {X_filt.shape[0]} rows")
    
    Dc = np.array(Dc)
    s_result_fci_jci = "FCI_JCI\n"
    s_result_coco = "COCO\n"

    for confounder_i, confounder in [(8, 8)]:  
        ...
        class cls:
            def __init__(self, G, n_contexts=10):
                self.G = G
                self.G_true = G 
        ...
        for i in range(len(fci.G_estim_bg.graph)):
           for j in range(len(fci.G_estim_bg.graph)):
        ...
        for i in range(len(results)):
            ...
            nms_observed = [nm for idx, nm in enumerate(nms) if idx != confounder]
            nm = nms
            ...
            if len(results[i].estimated_cuts):
                print("\tCUTS:", results[i].estimated_cuts, [nm[j] for j in results[i].estimated_cuts[0]])
